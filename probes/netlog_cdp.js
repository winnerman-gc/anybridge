...
(async () => {
    ...
    page.on('response', async res => {
        ...
        const url = res.url();
        // Script bundles routinely contain the word "calls" and dwarf the answer
        // stream, so they must never be candidates.
        if (/\.(png|jpe?g|svg|woff2?|css|riv|ico|mp4|js|mjs|wasm|map)(\?|$)/i.test(url)) return;
    ...
    const interesting = seen.filter(r => r.bytes || /event-stream|ndjson/i.test(r.ct));
    console.log('\n--- candidates ---');
    for (const r of interesting.sort((a, b) => (b.bytes || 0) - (a.bytes || 0)).slice(0, 15))
        console.log(String(r.bytes || 0).padStart(8), r.method.padEnd(5), r.ct.padEnd(34), r.url.slice(0, 100));

    // Rank by how stream-shaped a response is, not by size - the answer stream
    // is often the SMALLEST thing in the list.
    const score = b => (/event-stream|ndjson|json-stream|connect\+json/i.test(b.ct || '') ? 4 : 0)
                     + (/completion|conversation\b|\/responses|StreamGenerate/i.test(b.url) ? 2 : 0)
                     + (/calls|probe_net/.test(b.text) ? 1 : 0);
    const best = bodies.slice().sort((a, b) => (score(b) - score(a)) || (b.bytes - a.bytes))[0];
    if (best) {
        fs.writeFileSync(path.join(FIXTURES, `capture_${NAME}.txt`), best.text);
